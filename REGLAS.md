...
- **En el juego hoy:** Apertura **18 fechas** (bye) + **playoffs** (repechaje a partido, cuartos/semis/final ida y vuelta, sin goles de visita). El Apertura regular **no entrega estrella**: el título se define en los playoffs. Al coronar, Clausura de **otras 18** con tabla desde 0 y localías invertidas, otra vez con playoffs. Cobreloa = `CBL`, Cobresal = `CBS` (no chocar con Cobresal 2026 = `COB`). Descenso: tabla anual.
- Planteles documentados: **Colo-Colo, U. de Chile, Audax Italiano y Católica** (2006). El resto, cantera. Playoffs estilo México: **jugables** (Wikipedia 17 sep 2026).
